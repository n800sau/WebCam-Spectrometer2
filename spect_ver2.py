...
class Spect(QMainWindow,gui_spectver2.Ui_MainWindow):
	def __init__(self,parent=None):
		...
		self.img = cv2.imread(sys.argv[1]) if len(sys.argv) > 1 else None
		...
		self.f_ROI.addItem(self.raw_image)	#ROI Video
		...
		self.f_ROI.installEventFilter(self)
...
#		ad = '{}-{}-{}-{}-{}-{}'.format(veri[0],veri[1],veri[2],veri[3],veri[4],veri[5])
	...
	def updateImage(self):
		if not self.img is None:
			t = True
			frame = cv2.resize(self.img, (320, 240))
		else:
